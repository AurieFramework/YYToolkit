...
YYTKStatus API_Initialize()
{
	...
	{
		AUMIFunctionInfo Info; RValue Result;
...
DllExport YYTKStatus GetFunctionByIndex(int index, AUMIFunctionInfo& outInfo)
{
	if (index < 0)
		return YYTK_INVALID;

	gAPIVars.Code_Function_GET_the_function(index, &outInfo.Name, (PVOID*)&outInfo.Function, &outInfo.Arguments, 0);
...
DllExport YYTKStatus GetFunctionByName(const char* Name, AUMIFunctionInfo& outInfo)
{
	int Index = 0;
...
DllExport YYTKStatus GetGlobalInstance(YYObjectBase** ppoutGlobal)
{
	...
	*ppoutGlobal = (YYObjectBase*)Result.Pointer;
