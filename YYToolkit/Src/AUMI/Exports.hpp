...
DllExport AUMIResult AUMI_CreateCode(CCode* outCode, void* CodeBuffer, int CodeBufferSize, int LocalVarsUsed, const char* Name);
...
DllExport AUMIResult AUMI_GetFunctionByIndex(int index, struct AUMIFunctionInfo* outInformation);

// Get the AUMIFunctionInfo struct of a function (identified by name, case insensitive).
DllExport AUMIResult AUMI_GetFunctionByName(const char* Name, struct AUMIFunctionInfo* outInformation);

// Get the AUMIFunctionInfo struct of a function (identified by a pointer to the function)
DllExport AUMIResult AUMI_GetFunctionByRoutine(PFUNC_TROUTINE Routine, struct AUMIFunctionInfo* outInformation);
