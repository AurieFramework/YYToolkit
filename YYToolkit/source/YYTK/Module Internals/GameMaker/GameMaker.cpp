	...
	namespace Internal
	{
		...
		)
		{
			...
			for (auto& instruction : instructions)
			{
				...
				if (instruction.Instruction.info.operand_count != 2)
					continue;

				ZydisDecodedOperand& first_operand = instruction.Instruction.operands[0];
				ZydisDecodedOperand& second_operand = instruction.Instruction.operands[1];
				...
				ZyanUPointer rip = instruction.Instruction.runtime_address;

				// It's a pointer to a pointer, we dereference it once to   
				// get the actual pointer to the first element in the array
				RFunction** loaded_address = reinterpret_cast<RFunction**>(
					rip + instruction.Instruction.info.length + second_operand.mem.disp.value
				);

				*FunctionsArray = loaded_address;
				return YYTK_SUCCESS;
			}
