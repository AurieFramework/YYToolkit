...
void Framework::GUI::ApplyStylingDark()
{
...
void Framework::GUI::StyleFonts(ImGuiIO& _refIO)
{
...
std::string Framework::Wrappers::RValueKind_ToString(const YYRValue& _Value)
{
...
YYRValue Framework::Wrappers::GetGlobal(const char* _Name)
{
...
std::vector<std::string> Framework::Wrappers::GetVariableNames(long _InstanceID)
{
	using PFN_CALLBUILTIN = YYTKStatus(__cdecl*)(const char* _name, int _argc, YYRValue& _result, YYRValue* _args);
	static auto Global_CallBuiltin = g_pCurrentPlugin->GetCoreExport<PFN_CALLBUILTIN>("Global_CallBuiltin");

	YYRValue Storage[4];
	YYTKStatus Status = YYTK_FAIL;

	// variable_instance_get_names scope
	{
		double RealInstanceID = static_cast<double>(_InstanceID);
		YYRValue Argument = { RealInstanceID };
		Status = Global_CallBuiltin("variable_instance_get_names", 1, Storage[0], &Argument);
	}
	
	// If the function failed for whatever reason
	if (Status != YYTK_OK)
		return { "<Failed to get names>" };

	Status = Global_CallBuiltin("array_length_1d", 1, Storage[2], &Storage[0]);

	// If the function failed for whatever reason
	if (Status != YYTK_OK)
		return { "<Failed to get array length>" };

	if (!static_cast<bool>(Storage[2]))
		return { "<Empty>" };

	std::vector<std::string> Results;

	for (int n = 0; n < static_cast<int>(Storage[2]); n++)
	{
		// array_get scope
		{
			Storage[1] = YYRValue(static_cast<double>(n));

			Global_CallBuiltin("array_get", 2, Storage[3], Storage); // First two args are the array and 'n'

			if (Storage[3].As<RValue>().Kind == VALUE_STRING)
			{
				if (const char* pString = Storage[3].As<RValue>().String->Get())
					Results.push_back(pString);
			}
		}
	}
